  ...
  <div id="app">
    ...
    <div v-if="stocks.length" class="stock-list">
      <div class="stocks-grid">
        <div v-for="stock in stocks" :key="stock.id" class="stock-item">
          ...
          <button @click="addToWishlist(stock.id)" class="btn add-btn">Add to Wishlist</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="wishlist.length" class="wishlist">
      ...
      <div class="wishlist-grid">
        <div v-for="item in wishlist" :key="item.stock_id" class="wishlist-item">
          <span class="wishlist-name">{{ item.name }}</span>
          <button @click="removeFromWishlist(item.stock_id)" class="btn remove-btn">Remove</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      stocks: [],
      wishlist: [],
      userId: 1 // For simplicity, using a static user ID
    };
  },
  mounted() {
    this.fetchStocks();
  ...
  methods: {
    ...
    async fetchWishlist() {
      try {
        const response = await axios.get(`http://127.0.0.1:5000/wishlist/${this.userId}`);
